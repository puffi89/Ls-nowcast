...
# Zertifikategeschaeft der AG. NNLS auf 20 Quartale gegen das gemeldete
...
def fenster(df: pd.DataFrame, bruch: str) -> dict:
    """Tagesraten ueber verschiedene Beobachtungsfenster plus Streuung.

    Kurze Fenster reagieren schnell, rauschen aber stark. Lange Fenster sind
    ruhig, hinken einem Bruch aber hinterher. Die Wahl ist ein Tausch, kein
    Optimum - deshalb werden alle ausgewiesen.
    """
    g = df[(df.datum >= bruch) & df.ht]
    if g.empty:
        return {}
    sd = float(g.kassa.std()) if len(g) > 1 else 0.0
    out = {'streuung_mio': round(sd, 1), 'handelstage': len(g)}
    for n, name in [(5, 'tage5'), (10, 'tage10'), (20, 'tage20')]:
        if len(g) >= min(n, 3):
            out[name] = round(float(g.kassa.tail(n).mean()), 1)
    out['seit_bruch'] = round(float(g.kassa.mean()), 1)
    out['ewma'] = round(run_rate(df, bruch), 1)
    out['standardfehler_mio'] = round(sd / np.sqrt(len(g)), 1) if len(g) else None
    return out
...
def main(quelle: str, bruch: str = "2026-07-02", bp_ref: float = 5.4, sp_he: float = None):
    df = lade(quelle)
